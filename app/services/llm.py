...
class UIAnalyzer:
    ...
    def _generate_multiple_screens(self, content_analysis: Dict) -> list:
        """Generate multiple screens based on PDF features (NOT document headings)"""
        screens = []
        
        features = content_analysis.get('features', [])
        colors = content_analysis.get('colors', {})
        
        # Create screen names from FEATURES (not sections/headings)
        screen_names = []
        
        # Use features as screen names (these are actual app features, not document headings)
        for feature in features[:5]:
            # Convert feature to screen name
            screen_name = feature if not feature.endswith('s') else feature
            screen_names.append(f"{screen_name} Screen")
        
        # Ensure minimum screens
        if len(screen_names) < 3:
            screen_names.extend([f"{content_analysis['app_type']} Dashboard", "Details View", "Settings"])
        
        screen_names = screen_names[:5]  # Max 5 screens
        
        for i, screen_name in enumerate(screen_names):
            # Generate unique content for each screen based on PDF features
            screen_title = content_analysis['project_name'] if i == 0 else features[i] if i < len(features) else screen_name
            screen_subtitle = f"Your {content_analysis['app_type']} solution" if i == 0 else f"Manage {features[i]}" if i < len(features) else "Details"
            
            # Use actual features for each screen
            screen_features = features if i == 0 else features[i:i+3] if i < len(features) else features[:3]
            if len(screen_features) < 3:
                screen_features.extend([f"Option {j+1}" for j in range(3-len(screen_features))])
                ...
                "layout": {
                    "sections": [
                        ...
                        },
                        {
                            ...
                            "items": screen_features[:3] if i == 0 else None,
                            "grid_columns": 2 if i != 0 else None,
                            "cardTitle": features[i] if i < len(features) and i != 0 else None,
                            "gradient": f"linear {colors.get('secondary', '#8B5CF6')} → {colors.get('accent', '#06B6D4')}" if i != 0 else None
                        },
                        {
                            "component": "elevated_container",
                            "title": features[i+1] if i+1 < len(features) else "Details",
